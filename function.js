...
`;
  ...
  async function processSubElementsAndSendMessages(itemId) {
    try {
      ...
      for (const subitem of itemData.subitems) {
        ...
        // Send email
        await sendCustomEmail(email, `🥳 ${firstname}, ¡Felicidades! Tu contrato ya está autorizado ante notario.`, firstname, address, email, tipoContrato);
      }
  ...
  exports.mailContratoAutorizadoOtros = async (req, res) => {
      try {
